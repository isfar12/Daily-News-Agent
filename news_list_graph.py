...
def news_list_function(state: ArticleGraph) -> ArticleGraph:
    ...
    news_list = news_list_tool.invoke({"top_n": str(state["top_n"]), "user_input": state["input"]})
    return {
        "news_list": news_list
    }
...
def n_news(user_input: str):
    ...
    prompt=PromptTemplate(
        ...
        template='''Extract the number mentioned from this user query: 
        
        {user_input}

TASK:
Find the number(digit or word) eg. "3", "ten", "fifth" and convert to digit.

EXAMPLES:
- "What are today's top 15 news?" -> number: "15"
- "I want to read some news" -> number: "10" 
- "Top eight news" -> number: "8"
- "Show me fifth news" -> number: "5"
- "What are the top 10 news today?" -> number: "10"

If no number is found, return "10" as default.

Extract the number and provide it in the required format.'''
    )
...
def get_news_list(user_input: str, thread_id: str = None):
    """Get a list of today's news headlines. Use when user wants to see multiple news articles or general news overview.
    
    Use for: "today's news", "latest headlines", "top 10 news", "sports news", "show me news"
    Don't use for: specific article requests with numbers/positions
    """
    if thread_id is None:
        thread_id = THREAD_ID
...
# result=get_news_list.invoke({"user_input": "today top eight news", "thread_id": "test-thread"})
# print(result)